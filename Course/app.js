...
// ~TSX.
// ~TSX es una librería diseñada para ejecutar archivos TypeScript de manera directa, sin necesidad de realizar manualmente una compilación previa a JavaScript cada vez que hacemos un cambio. Esto la convierte en una herramienta muy útil durante el desarrollo, ya que automatiza el proceso de ejecución y reinicio del servidor cuando detecta modificaciones en los archivos TypeScript. En otras palabras, su comportamiento es muy similar al de Nodemon, pero enfocado específicamente en proyectos TypeScript. Gracias a esto, evita que tengamos que depender constantemente de `tsc --watch` para compilar el código o de pasos adicionales para ejecutar el archivo resultante. Es una paquetería especialmente práctica cuando queremos una experiencia de desarrollo rápida, fluida y orientada a TypeScript. Para instalarla normalmente se utiliza el nombre `tsx`, ya que ese es el paquete que se agrega al proyecto. Ademas esta comunmente se instala como devDependence devido a que solo sera ejecutada durante las pruebas de desarrollo.
// npm i --save-dev tsx
...
//     "tsx": "^4.21.0" //* Ejecuta TypeScript directamente con reinicio automático.
//  },
...
// ~Inicializacion de TypeScript en proyectos Node.JS.
// ~Una vez que ya tenemos instalados TypeScript y sus dependencias relacionadas con el tipado, el siguiente paso es inicializar correctamente el entorno para comenzar a trabajar con archivos `.ts` dentro de un proyecto Node.JS. Esta inicialización es importante porque TypeScript no solo necesita estar instalado, sino también configurado para saber **cómo, dónde y desde qué carpeta** va a leer los archivos fuente y hacia dónde va a generar el código JavaScript compilado. En proyectos de Node.JS esto cobra todavía más importancia, porque Node normalmente ejecuta JavaScript y no TypeScript de forma directa, por lo que necesitamos una configuración clara que permita transformar nuestros archivos TypeScript en JavaScript antes de ejecutarlos. Para ello se utiliza el archivo `tsconfig`, que contiene las reglas principales de compilación y comportamiento del proyecto. En el caso de Node.JS, esta inicialización se realiza comúnmente mediante `npx`, ya que `npx` permite ejecutar herramientas instaladas dentro del proyecto sin depender necesariamente de instalaciones globales. Por eso, el comando estándar para generar esta configuración base es `npx tsc --init`, el cual crea el archivo `tsconfig.json` con una configuración inicial que después podremos modificar según nuestras necesidades.
// npx tsc --init  //* Inicializa TypeScript dentro del proyecto y genera el archivo tsconfig.json con la configuración base de compilación.

// ~Declaracion de ubicacion de archivos TypeScript y JavaScript.
// ~Como bien sabemos, dentro del archivo `tsconfig` se pueden realizar múltiples configuraciones, pero en el contexto de TypeScript con Node.JS hay dos rutas que resultan especialmente importantes: la carpeta donde se encuentra el código fuente en TypeScript y la carpeta donde se guardarán los archivos JavaScript compilados. Esto es fundamental porque en un proyecto bien estructurado no queremos mezclar el código fuente con el código ya transpilado, ya que eso dificulta el orden, la lectura y el mantenimiento del proyecto. Por esta razón se suelen declarar dos propiedades muy importantes: `rootDir`, que indica dónde vive el código TypeScript original, y `outDir`, que indica dónde se colocarán los archivos JavaScript generados por el compilador. Esta separación permite mantener una estructura limpia y profesional, algo especialmente útil en proyectos backend más grandes. Cabe destacar que estos ajustes e instrucciones aplican solo cuando se ejecuta la copilacion mediante "npx tsc".
// ?--outDir.
// ?`--outDir` especifica la carpeta de salida o *output directory*. En esta carpeta TypeScript colocará los archivos `.js` ya compilados a partir del código `.ts`. Normalmente esta carpeta se llama `dist/` o `build/`, y suele ser la carpeta desde la cual Node.JS ejecuta el proyecto ya transpilado. Esto es importante porque el código fuente no debe ejecutarse directamente en Node sin pasar primero por la compilación correspondiente. Mediante `--outDir direccionRuta` podemos definir exactamente dónde se guardará el código JavaScript generado, lo que nos ayuda a separar claramente la fuente del resultado compilado. Donde adicionalmente de forma automatica dentro de la carpeta que especifiquemos generara los archivos y los guardara en carpetas segun la estructura de carpetas que tengamos en el codigo fuente, por lo que si tenemos un archivo dentro de una carpeta src/controllers, al compilar se generara un archivo con el mismo nombre dentro de dist/controllers, manteniendo asi la misma estructura pero con los archivos ya compilados a JavaScript.

// ?--rootDir.
// ?`--rootDir` especifica la carpeta de entrada o carpeta raíz del código fuente. Esta es la ubicación donde vive el código TypeScript original (`.ts`) que será leído por el compilador. Normalmente se utiliza una carpeta llamada `src/`, ya que ahí se concentra todo el código editable del proyecto. El compilador de TypeScript tomará como referencia esa ruta para buscar los archivos que debe transformar a JavaScript. Mediante `--rootDir direccionRuta` podemos establecer claramente desde qué carpeta trabajará TypeScript, lo que evita desorden y facilita el control de la estructura del proyecto.
...
// ~Comando de ejecución de TSX (Entorno de Desarrollo).
// ~La declaración de un comando dentro de nuestro archivo package.json, el cual ejecutaremos posteriormente mediante npm run, debe estar pensada para cubrir ciertas características que nos permitan trabajar de la manera más óptima posible durante el desarrollo. En este caso se utiliza el comando tsx watch src/server.ts, donde tsx es el ejecutor moderno que sustituye a herramientas como tsx. Su función principal es interpretar archivos TypeScript de forma nativa y ultra rápida utilizando el motor de compilación esbuild. Al añadir el parámetro watch, la herramienta se queda "escuchando" cualquier cambio en el código fuente; en cuanto guardamos un archivo, el servidor se reinicia automáticamente de forma casi instantánea. Finalmente, se especifica la ruta del archivo principal de entrada, en este caso src/server.ts, que es el punto de inicio de la aplicación. Al igual que otras herramientas de desarrollo, tsx maneja la ejecución en memoria, lo que significa que no crea archivos .js físicos en tu carpeta de proyecto mientras programas, manteniendo todo limpio y ágil. Es la opción estándar para proyectos modernos que utilizan módulos de ECMAScript ("type": "module").
// tsx watch src/server.ts //* Comando de desarrollo que ejecuta TypeScript con recarga automática instantánea y punto de entrada en src/server.ts.

// ~Comando de compilacion de TSC.
// ~A diferencia de `tsx`, que trabaja con archivos TypeScript en memoria y permite ejecutarlos de manera directa durante el desarrollo, el comando `npx tsc` se utiliza para generar archivos JavaScript físicos. Esto significa que TypeScript toma los archivos `.ts` ubicados en la carpeta definida como entrada, los compila y escribe su contraparte `.js` dentro de la carpeta de salida previamente configurada mediante `--outDir` y `--rootDir`. Es decir, aquí ya no hablamos de ejecución en memoria, sino de una compilación real y persistente en el sistema de archivos. Esto es especialmente importante porque, en producción, Node.JS no suele ejecutar TypeScript directamente, sino los archivos JavaScript ya compilados. Por esa razón, en el archivo `package.json` normalmente se especifica en la propiedad `main` la ruta del archivo JavaScript generado, para que en producción Node lea desde esa ubicación. En otras palabras, `tsx` se usa principalmente durante desarrollo, mientras que `tsc` se usa para compilar el proyecto de forma física y dejarlo listo para su ejecución final.
// npx tsc. //* Comando que compila TypeScript y genera los archivos JavaScript físicos en la carpeta de salida configurada.

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// ^Tipado de TypeScript dentro de proyectos de Node.js.
// ^Como bien sabemos, una de las características más importantes de TypeScript es su **tipado estático**, y precisamente ese sistema de tipos es una de las razones por las cuales TypeScript se vuelve tan útil en proyectos de Node.JS. Gracias a este tipado, podemos definir con mayor precisión qué tipo de datos esperamos recibir, manejar y devolver dentro de nuestras funciones, rutas, middlewares y controladores. Sin embargo, cuando trabajamos con Node.JS, existen ciertos casos en los que el tipado debe adaptarse específicamente a las herramientas del ecosistema, especialmente cuando usamos Express. Esto sucede porque muchas de las funciones que utilizamos en el backend, como `req`, `res`, `next`, controladores y middlewares, necesitan ser tipadas de forma correcta para que TypeScript pueda reconocerlas, validar sus propiedades y evitar errores durante el desarrollo. Por eso, dentro de proyectos Node.JS con TypeScript, existen tipados muy concretos que se utilizan para asegurar compatibilidad entre ambos lenguajes y mantener un código más seguro, explícito y mantenible.

// ~Tipado de request y response dentro de rutas, middlewares, controllers, entre otros dentro de Node.js.
// ~Bien sabemos que los parámetros `req` y `res` dentro de nuestros proyectos Node.JS son una parte fundamental de las rutas y, por lo tanto, también de los servidores que estamos construyendo. `req` representa la información de la petición que llega desde el cliente, mientras que `res` representa la respuesta que el servidor enviará de vuelta. Como estamos trabajando con TypeScript, no basta con declarar simplemente estos parámetros, sino que es necesario tiparlos correctamente para que el compilador conozca qué tipo de objeto estamos utilizando. En el caso de Express, este tipado ya viene preparado dentro del propio paquete, por lo que podemos importar los tipos `Request` y `Response` directamente desde `express`. De esta manera, TypeScript podrá reconocer las propiedades y métodos disponibles sobre cada uno de estos objetos, como `req.body`, `req.params`, `req.query`, `res.json()`, `res.status()`, entre otros. Para el caso de `next` dentro de middlewares, también debemos tiparlo con `NextFunction`, ya que es una función especial de Express que indica la continuación del flujo de ejecución.
