...
// ~Comando de ejecucion de Ts Node Dev.
// ~La declaración de un comando dentro de nuestro archivo `package.json`, el cual ejecutaremos posteriormente mediante `npm run`, debe estar pensada para cubrir ciertas características que nos permitan trabajar de la manera más óptima posible durante el desarrollo. En este caso se utiliza un comando como `tsnd --respawn --clear src/app.ts`, donde `tsnd` hace referencia a la paquetería `ts-node-dev`, la cual cumple una función muy similar a Nodemon, pero diseñada específicamente para proyectos TypeScript. Su objetivo principal es detectar cambios en los archivos `.ts`, reiniciar automáticamente la ejecución y mantener un flujo de trabajo mucho más ágil. El siguiente parámetro, `--respawn`, le indica a `ts-node-dev` que vuelva a lanzar el proceso cada vez que detecte cambios en el código, mientras que `--clear` se encarga de limpiar la consola para que la salida se vea ordenada cada vez que el programa se reinicia. Finalmente, se especifica la ruta del archivo principal de entrada, en este caso `src/app.ts`, que será el punto de inicio de la aplicación. La gran ventaja de este comando es que los archivos JavaScript no se generan físicamente en cada cambio, sino que se manejan en memoria, lo cual evita sobrecargar el proyecto con archivos temporales innecesarios. Cuando se guarda un cambio, la versión anterior en memoria se reemplaza automáticamente por la nueva, y esa nueva versión es la que Node.JS termina ejecutando. Por eso esta librería resulta tan útil en desarrollo: actúa como un puente entre TypeScript y Node, permitiendo ejecutar el código sin tener que compilar manualmente en cada modificación.
// tsnd --respwan --clear src/app.ts  //* Comando de desarrollo que ejecuta TypeScript con recarga automática, limpieza de consola y punto de entrada en `src/app.ts`.

// ~Comando de compilacion de TSC.
// ~A diferencia de `ts-node-dev`, que trabaja con archivos TypeScript en memoria y permite ejecutarlos de manera directa durante el desarrollo, el comando `npx tsc` se utiliza para generar archivos JavaScript físicos. Esto significa que TypeScript toma los archivos `.ts` ubicados en la carpeta definida como entrada, los compila y escribe su contraparte `.js` dentro de la carpeta de salida previamente configurada mediante `--outDir` y `--rootDir`. Es decir, aquí ya no hablamos de ejecución en memoria, sino de una compilación real y persistente en el sistema de archivos. Esto es especialmente importante porque, en producción, Node.JS no suele ejecutar TypeScript directamente, sino los archivos JavaScript ya compilados. Por esa razón, en el archivo `package.json` normalmente se especifica en la propiedad `main` la ruta del archivo JavaScript generado, para que en producción Node lea desde esa ubicación. En otras palabras, `ts-node-dev` se usa principalmente durante desarrollo, mientras que `tsc` se usa para compilar el proyecto de forma física y dejarlo listo para su ejecución final.
